...
public class SearchMedicalFiles extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int patient_type = getPatientType(firstname, lastname);
        ...
        if (checkFirstname && checkLastname) {
            try {
                if (patient_type == 0) {
                    ...
                    String sql = "SELECT"
                               + " DATE_FORMAT(pr.visit_date, '%m/%d/%Y'), p.id, u.first_name, u.middle_name, u.last_name, pr.disease_name,"
                               + " pr.treatment, pr.medicine_given, pr.medicine_name, pr.ambulance_service_used, pr.billing_amount, pr.medical_notes"
                               + " FROM users u"
                               + " INNER JOIN"
                               + " (SELECT id, user_id FROM patients) p"
                               + " ON u.id = p.user_id"
                               + " INNER JOIN"
                               + " (SELECT visit_date, patient_id, disease_name, treatment,"
                               + " medicine_given, medicine_name, ambulance_service_used, billing_amount, medical_notes"
                               + " FROM patient_records) pr"
                               + " ON p.id = pr.patient_id"
                               + " WHERE u.first_name = ? AND u.last_name = ?"
                               + " ORDER BY pr.visit_date";
                    ...
                    if (rs.next()) {
                        ...
                        while (rs.next()) {
                            ...
                            array[3] = rs.getString(4);   // users.middle_name
                            array[4] = rs.getString(5);   // users.last_name
                            array[5] = rs.getString(6);   // patient_records.disease_name
                            array[6] = rs.getString(7);   // patient_records.treatment
                            if (rs.getBoolean(8) == true) {    // patient_records.medicine_given
                                array[7] = "Yes";
                            } else if (rs.getBoolean(8) == false) {
                                array[7] = "No";
                            }
                            array[8] = rs.getString(9);   // patient_records.medicine_name
                            if (rs.getBoolean(10) == true) {    // patient_records.ambulance_service_used
                                array[9] = "Yes";
                            } else if (rs.getBoolean(10) == false) {
                                array[9] = "No";
                            }
                            array[10] = rs.getString(11);   // patient_records.billing_amount
                            array[11] = rs.getString(12);   // patient_records.medical_notes
                            map.put(i, array);
                            i++;
                        }
                        validationMap.put("search", "successful");
    ...
    protected int getPatientType(String firstname, String lastname) {
        try {
            String sql = "SELECT type FROM patients WHERE user_id"
